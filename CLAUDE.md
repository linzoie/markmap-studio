...
>
> **2026-07-09 狀態更新**：Stop hook 已統一為工作區範本 v2.1 版（`sync-governance.ps1` 管理）；
> 新增 `test/smoke.test.js`（node:test，離線 ~2 秒：markmap-lib Transformer 轉換、
> SAMPLE_MD、standalone 模板佔位符三類回歸）與 `package.json` 的 `test` script，
> Stop hook 會自動跑它。`pnpm build`（43 秒）維持不納入，靠 DoD 手動確認 + CI 兜底。
...
- `pnpm test` **全綠**（`test/smoke.test.js`：Transformer 轉換 + SAMPLE_MD + 模板佔位符；
  `test/property.test.js`：決定性／結構保持／空輸入／回歸錨點 metamorphic 不變式，零依賴種子模糊）
...
> Stop hook `.claude/hooks/verify-before-done.ps1` 是**工作區範本版**（由 `sync-governance.ps1`
> 管理，非客製）：因本專案有 `test` script，Stop hook 會自動跑 `node --test`（smoke +
> property，~1.5 秒）並解析 pass/fail 計數（0 個測試會被擋）。prettier 由 after-edit hook
> 自動跑，非 Stop hook 職責。`pnpm build`（43 秒）仍不納入 Stop，靠 CI（push 觸發）+ DoD
> 手動確認雙重把關。
>
> （2026-07-11 裁定：本段原記載「客製版只跑 prettier」與 2026-07-09 狀態更新「已統一為
> 範本 v2.1」矛盾；以磁碟為權威——實掛範本版會跑 node --test，已改齊。）
